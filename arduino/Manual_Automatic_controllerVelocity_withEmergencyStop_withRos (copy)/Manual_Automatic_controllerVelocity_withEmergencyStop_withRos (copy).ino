...
// Pin al que está conectado el switch de paro total
const int switchPin = 12; // Pin al que está conectado el switch
const int LEDPin = 13; // Pin al que está conectado el switch
...
ros::Publisher info_pub("/info", &info_msg);
...
void setup() {
  ...
  //setUp Switch paro total
  pinMode(switchPin, INPUT_PULLUP); // Configura el pin del switch como entrada con resistencia pull-up
...
void loop() {
  ...
  //while de paro, si el swtich de paro se activa se queda cicaldo en esta parte
  while (digitalRead(switchPin)==LOW) { // Si el switch está presionado (lectura en bajo)
    //Serial.println("Apagando el Arduino...");
    delay(100); // Espera un momento antes de apagar
    //Se prende el led que indica el encendido del switch
    digitalWrite(LEDPin, HIGH);
    
    //pines conectados a puente H
    digitalWrite(2, LOW);
    digitalWrite(3, LOW);
    digitalWrite(4, LOW);
    digitalWrite(5, LOW);
    ...
    //pines conectados a bluetooth
    digitalWrite(10, LOW);
    digitalWrite(11, LOW);
    
    // Apagar el LED "ON" (LED de alimentación)
    delay(100);
    asm volatile ("jmp 0"); // Reiniciar el Arduino (necesita reinicio externo)
  }
...
void joystick_read(){
  ...
  while (btSerial.available()) {
    ...
    if (c == ',') {
      ...
      valorStr = "";
    } else if (c == '\n') {  // Fin de línea indica el final del número.
      boton = valorStr.toInt();
      //Serial.println(boton); // Mostrar el valor entero en el Monitor Serial.
      valorStr = "";
    } else {
      valorStr += c;
    }
